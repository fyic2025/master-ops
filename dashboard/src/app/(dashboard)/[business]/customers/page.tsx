...
function ElevateCustomerManager() {
  ...
  const stats = {
    ...
    activated: customers.filter(c => c.state === 'ENABLED').length,
...
function CustomerList({
  ...
  const pendingSelected = customers.filter(c => selectedIds.has(c.id) && c.state === 'INVITED')
    ...
    <div className="space-y-4">
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-2">
            ...
            {pendingSelected.length > 0 && (
              <button
                onClick={() => sendActivationEmails(pendingSelected.map(c => c.id))}
              ...
              >
                ...
                Send Activation ({pendingSelected.length})
              </button>
            )}
          </div>
        )}
      ...
      <div className="flex items-center gap-6 text-sm text-gray-500">
        <span className="flex items-center gap-2">
          ...
          Activated = Can place orders
        </span>
        <span className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-yellow-400"></span>
          Pending = Needs to click activation link
        </span>
        <span className="flex items-center gap-2">
...
function CustomerRow({
  ...
  return (
    <tr className="hover:bg-gray-800/50">
      ...
      <td className="px-4 py-3">
        <span className={`px-2 py-1 rounded text-xs font-medium ${getStateColor(customer.state)}`}>
