-- =============================================================================
-- BUY ORGANICS ONLINE - CATEGORY TABLES (Quick Setup)
-- =============================================================================
-- Run this in Supabase Dashboard SQL Editor:
-- https://supabase.com/dashboard/project/usibnysqelovfuctmkqw/sql/new
-- =============================================================================

-- 1. Create bc_categories table
...
-- 2. Create product_category_links table
...
-- 3. Create category_analysis table
...
-- Low product categories (for finding redundant categories)
...
-- Content gaps (missing SEO content)
...
SELECT 'total_categories' AS metric, COUNT(*)::TEXT AS value FROM bc_categories WHERE is_active = TRUE
UNION ALL SELECT 'root_categories', COUNT(*)::TEXT FROM bc_categories WHERE parent_id = 0 AND is_active = TRUE
UNION ALL SELECT 'empty_categories', COUNT(*)::TEXT FROM bc_categories WHERE product_count = 0 AND is_active = TRUE
UNION ALL SELECT 'hidden_categories', COUNT(*)::TEXT FROM bc_categories WHERE is_visible = FALSE AND is_active = TRUE
UNION ALL SELECT 'categories_without_description', COUNT(*)::TEXT FROM bc_categories WHERE (description IS NULL OR LENGTH(description) < 50) AND is_active = TRUE
UNION ALL SELECT 'categories_without_meta_description', COUNT(*)::TEXT FROM bc_categories WHERE (meta_description IS NULL OR LENGTH(meta_description) < 50) AND is_active = TRUE
UNION ALL SELECT 'average_products_per_category', ROUND(AVG(product_count)::NUMERIC, 2)::TEXT FROM bc_categories WHERE is_active = TRUE AND product_count > 0
UNION ALL SELECT 'max_tree_depth', MAX(tree_depth)::TEXT FROM bc_categories WHERE is_active = TRUE;

-- Done!
SELECT 'Category tables and views created successfully!' AS status;
